# Sir Renal

![Language](https://img.shields.io/static/v1?label=language&message=castellano%20%7C%20english%20%7C%20italian%20%7C%20russian%20%7C%20&color=informational)

**Autor** : **Eslizon**

[Link to original Mod Download](http://www.shsforums.net/files/file/854-sir-renal-v25/)

For Baldur's Gate II,  Baldur's Gate II: EE, Baldur's Gate Trilogy (BGT) and the Enhanced Edition Trilogy (EET).


## Description:
---------------

A mod from Clan REO

Sir Renal is in charge of negotiating the items of the Order of the Most Radiant Heart and can be found within the order HQ (area = ar0903 X = 1224 Y = 477).

Renal, besides offering services typical of temples, sells the following new goods:

Ring of Wisdom
Ring of Greater Holiness
Sacred Helm
Sacred Plate Mail
Chain Mail +1: Protected against Evil
Mace of the Celestial Fire
Bastard Sword +1: Justice
Sir Errant's Virtue
False Dragonscale Armor +3: Blue Mountain
Medium Shield +2: Magebane



## Install:
--------

Extract archive contents into your game directory
Run setup-Renal.exe
Follow the prompt


## Version History:
----------------

v2.6
- EE/EET compatibility
- Gw function for Bam and Description
- IconV Autotra
- Sir Renal position from X = 1062 Y = 417 to X = 1224 Y = 477
- Correction for ESLISWO2.ITM and ESLISWO3.ITM
- Readme.MD
- Metadata / Label   
- InfinityAutoPackager  

v2.5
- Añadida traducción al ruso por Silent

v2.4
- Añadida traducción al italiano por Ilot

v2.3
- Textos corregidos

v2.2
- Añadida traducción al ingles por Lollorian

v2.1
- Updated WeiDU to v211
- Traified mod
- Corrected text